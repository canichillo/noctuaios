...
@interface NODescargasViewController ()
@property (nonatomic, strong) M13ProgressHUD * progreso;
@end

@implementation NODescargasViewController
...
-(void) initUI
{
    ...
    [SPUtilidades crearBarraNavegacion:self Titulo:@"Descargas" MenuIzquierdo:NO MenuDerecho:NO SelectorIzquierdo:@selector(volverAtras:)];
}
...
-(void) cargarDatos
{
    ...
    [SPUtilidades procesarPeticionPOST: @"descargasoferta"
                            Parametros: params
                          ...
                          FailureBlock: ^(NSError *error)
     {
         ...
         [SPUtilidades mostrarError:@"Error" Mensaje:@"No se ha podido obtener los datos de las descargas" Handler:^(SIAlertView *alertView) { }];
     }
     ];
}
...
{
    NOSeguidorTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"Descarga"];
    
    if (!cell)
    {
        cell = [[NOSeguidorTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault
                                              reuseIdentifier:@"Descarga"];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([seguidor.amigo isEqual:@""])
        botones = @[@"Solicitar Amistad", @"Chatear"];
    else
    {
        if ([seguidor.amigo isEqual:@"A"]) botones = @[@"Ver sus ofertas", @"Chatear"];
        if ([seguidor.amigo isEqual:@"E"]) botones = @[@"Eliminar solicitud", @"Chatear"];
